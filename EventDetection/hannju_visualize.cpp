...
/************************************************************************
 ...
 Return Values:
	- none
************************************************************************/
void hj::DrawBoxWithID(cv::Mat &imageFrame, cv::Rect box, unsigned int nID, int lineStyle, int fontSize, std::vector<cv::Scalar> *vecColors)
{
	// get label length
...
/************************************************************************
 ...
 Return Values:
	- none
************************************************************************/
void hj::Draw3DBoxWithID(cv::Mat &imageFrame, std::vector<cv::Point> &pointArray, unsigned int nID, std::vector<cv::Scalar> *vecColors)
{
	...
	{
		points.push_back(pointArray[drawOrder[pointIdx]]);
	}
	...
	{
		cv::line(imageFrame, pointArray[pointIdx], pointArray[pointIdx + 4], curColor);
	}
...
/************************************************************************
 ...
 Return Values:
	- none
************************************************************************/
void hj::DrawTriangleWithID(cv::Mat &imageFrame, cv::Point &point, unsigned int nID, std::vector<cv::Scalar> *vecColors)
{
	double size = 20;
	cv::Scalar curColor = hj::getColorByID(nID, vecColors);
	// draw five points
	...
	std::vector<cv::Point> points;
	cv::Point topPoint(point.x, point.y - (int)size);
	cv::Point leftPoint(point.x - (int)cosd30xSize, point.y + (int)sind30xSize);
	cv::Point rightPoint(point.x + (int)cosd30xSize, point.y + (int)sind30xSize);
	points.push_back(topPoint);
	points.push_back(leftPoint);
	points.push_back(rightPoint);
	points.push_back(topPoint);
	points.push_back(leftPoint); // for handling thick line
...
/************************************************************************
 ...
 Return Values:
	- none
************************************************************************/
void hj::DrawLine(cv::Mat &imageFrame, std::vector<cv::Point> &pointArray, unsigned int nID, int lineThickness, std::vector<cv::Scalar> *vecColors)
{
	cv::Scalar curColor = hj::getColorByID(nID, vecColors);
	for (int pointIdx = 0; pointIdx < (int)pointArray.size() - 1; pointIdx++)
	{
		cv::line(imageFrame, pointArray[pointIdx], pointArray[pointIdx + 1], curColor, 1 + lineThickness);
	}
}
...
/************************************************************************
 ...
 Return Values:
	- 
************************************************************************/
cv::Point hj::PointRescale(cv::Point _point, double _rescale, double _xmin, double _ymin)
{
	cv::Point resultPoint = _point;

	resultPoint.x -= (int)_xmin;
	resultPoint.y -= (int)_ymin;
