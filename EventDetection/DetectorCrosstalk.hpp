...
public:
	...
	bool Initialize();
	bool Finalize(void);
	vector<CDetectedObject> Detect(cv::Mat _targetImage, int _frameIndex);
...
private:
	...
	vector<CDetectedObject> CrosstalkComputeDetect(vector<mxArray *> &pyr_chn_concat);
	...
	vector<CDetectedObject> CrosstalkGetRawDetection(void);
	
	// non-maximal suppression
	vector<CDetectedObject> bbNms_maxg(vector<CDetectedObject> st_bbs, double overlap);
...
#undef PAD
	...
	//----------------------------------------------------------------
private:
	vector<CDetectedObject> m_RawDetectionResultVector;
	vector<CDetectedObject> m_DetectionResultVectorOneCam;
