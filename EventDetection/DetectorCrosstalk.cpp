...
inline bool SortDetectionScoreDescend(CDetectedObject &val1, CDetectedObject &val2)
{
...
CDetectorCrosstalk::Initialize()
{
	...
	char *str_detector_dir = "./data/detector";
	sprintf_s(file_opts,  "%s/opts.jwmat",  str_detector_dir);
	sprintf_s(file_clf,   "%s/clf.jwmat",   str_detector_dir);
	sprintf_s(file_info1, "%s/info1.jwmat", str_detector_dir);
	sprintf_s(file_info2, "%s/info2.jwmat", str_detector_dir);
	sprintf_s(file_info3, "%s/info3.jwmat", str_detector_dir);
...
CDetectorCrosstalk::Detect(cv::Mat _targetImage, int _frameIndex)
{
	...
	if (1.0 != IMAGE_RESCALE)
	{
		cv::resize(_targetImage, _targetImage, 
			cv::Size((int)((double)_targetImage.cols * IMAGE_RESCALE), 
			         (int)((double)_targetImage.rows * IMAGE_RESCALE)));
	}
	...
	m_DetectionResultVectorOneCam = bbNms_maxg(m_DetectionResultVectorOneCam, 0.4);
	...
	for(int idx = 0 ; idx < OriginalResultSize; ++idx) {
		if(m_DetectionResultVectorOneCam[count].score < 30) {
	...
	m_RawDetectionResultVector = m_DetectionResultVectorOneCam;
	m_DetectionResultVectorOneCam = bbNms_maxg(m_DetectionResultVectorOneCam, 0.25);
	...
	for(int idx = 0 ; idx < (int)m_DetectionResultVectorOneCam.size(); ++idx) {
		shrinkWidth = ((1.0-shrinkRatio)/2.0)*m_DetectionResultVectorOneCam[idx].bounding_box.width;
		shrinkHeight = ((1.0-shrinkRatio)/2.0)*m_DetectionResultVectorOneCam[idx].bounding_box.height;
		m_DetectionResultVectorOneCam[idx].bounding_box.x += shrinkWidth;
		m_DetectionResultVectorOneCam[idx].bounding_box.y += shrinkHeight;
		m_DetectionResultVectorOneCam[idx].bounding_box.width *= shrinkRatio;
		m_DetectionResultVectorOneCam[idx].bounding_box.height *= shrinkRatio;

		// recover size
		m_DetectionResultVectorOneCam[idx].bounding_box.x *= (1.0 / (double)IMAGE_RESCALE);
		m_DetectionResultVectorOneCam[idx].bounding_box.y *= (1.0 / (double)IMAGE_RESCALE);
		m_DetectionResultVectorOneCam[idx].bounding_box.width *= (1.0 / (double)IMAGE_RESCALE);
		m_DetectionResultVectorOneCam[idx].bounding_box.height *= (1.0 / (double)IMAGE_RESCALE);
	}
...
CDetectorCrosstalk::CrosstalkComputeDetect(vector<mxArray *> &pyr_chn_concat) {
	...
	for(int ii=0;ii < modified_scale;++ii) {
		for(int jj=1;jj<=nDs;jj++) {
			...
			for(int row=0; row<bb_size[0]; row++) {
				CDetectedObject st_temp;
				st_temp.bounding_box.x = temp_bb[row];
				st_temp.bounding_box.y = temp_bb[row + bb_size[0]];
				st_temp.bounding_box.width = temp_bb[row + bb_size[0]*2];
				st_temp.bounding_box.height = temp_bb[row + bb_size[0]*3];
...
// 코 //
vector<CDetectedObject>
CDetectorCrosstalk::bbNms_maxg(vector<CDetectedObject> st_bbs, double overlap)
{
	double thr = -1000;
	double overDnm = 1;
	double greedy = 1;
	...
	{
		kp[i] = true;
		as[i] = st_bbs[i].bounding_box.width * st_bbs[i].bounding_box.height;
	}
	...
	for(int i=0;i<st_bbs.size();i++)
	{
		...
		for(int j=i+1;j<st_bbs.size();j++)
		{
			...
			double iw = min(st_bbs[i].bounding_box.x+st_bbs[i].bounding_box.width,st_bbs[j].bounding_box.x+st_bbs[j].bounding_box.width)-max(st_bbs[i].bounding_box.x,st_bbs[j].bounding_box.x);
			if(iw<=0) continue;
			double ih = min(st_bbs[i].bounding_box.y+st_bbs[i].bounding_box.height,st_bbs[j].bounding_box.y+st_bbs[j].bounding_box.height)-max(st_bbs[i].bounding_box.y,st_bbs[j].bounding_box.y);
			if(ih<=0) continue;
			double o = iw*ih;
			double u;
	...
	vector<CDetectedObject> st_bbs_valid;
	for(int i=0;i<st_bbs.size();i++)
	{
